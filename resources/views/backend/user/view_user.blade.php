    ...
    <section class="content">
    	...
    	<div class="card card-primary card-outline">
    		...
    		<div class="card-header">
	            ...
	            <h3 class="card-title"><a href="{{ route('users.add') }}" class="btn btn-primary btn-block">
              	...
              	@foreach ($allData as $key => $user)
	              	...
	              	<tr>
	              		<td>{{ $key + 1 }}</td>
										...
										<td>{{ $user->usertype }}</td>
										<td>
                      <img id="showImage" src="{{ (!empty($user->image))? url('upload/user_images/'.$user->image):url('upload/user.jpg') }}" style="width: 60px;width: 60px;">          
                    </td>
										<td>{{ $user->status }}</td>
